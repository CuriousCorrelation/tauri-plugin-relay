...
fn execute_request(request: &Request, cancel_token: &CancellationToken) -> RequestResult<Response> {
    tracing::info!(
    ...
    let mut request = CurlRequest::new(&mut handle, request);
    request.prepare()?;
...
#[tracing::instrument(skip(request), fields(request_id = request.id), level = "debug")]
pub async fn execute(request: Request) -> RequestResult<Response> {
...
pub async fn cancel(request_id: i64) -> RequestResult<()> {
