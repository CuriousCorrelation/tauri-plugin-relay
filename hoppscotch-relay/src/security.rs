...
use crate::{
    ...
    interop::{CertificateType, Security, SecurityCertificates},
};
...
impl<'a> SecurityHandler<'a> {
    ...
    pub(crate) fn configure(&mut self, security: &Security) -> RequestResult<()> {
        tracing::trace!(
            validate_certs = security.validate_certificates,
            verify_host = security.verify_host,
            "Configuring security settings"
        );

        self.handle
            .ssl_verify_peer(security.validate_certificates)
            .map_err(|e| {
                tracing::error!(error = %e, "Failed to set SSL verify peer");
                InterceptorError::Certificate {
        ...
        self.handle
            .ssl_verify_host(security.verify_host)
            .map_err(|e| {
        ...
        if let Some(ref certs) = security.certificates {
            tracing::trace!("Configuring certificates");
    ...
    #[tracing::instrument(skip(self), level = "debug")]
    fn configure_certificates(&mut self, certs: &SecurityCertificates) -> RequestResult<()> {
        if let Some(ref client_cert) = certs.client {
            match client_cert {
                CertificateType::Pem { cert, key } => {
                    ...
                    self.handle
                        .ssl_cert_blob(cert)
                        .map_err(|e| InterceptorError::Certificate {
                ...
                CertificateType::Pfx { data, password } => {
                    ...
                    if let (Some(cert), Some(key)) = (parsed.cert, parsed.pkey) {
                        ...
                        tracing::trace!("Setting up converted PEM certificate and key");
                        self.handle.ssl_cert_type("PEM").map_err(|e| {
